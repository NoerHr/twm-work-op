...
export function EnhancedSidebar({ currentPage, onNavigate }: EnhancedSidebarProps) {
  ...
  // Group items by section (NEW: 3-group structure)
  const myZoneItems = allowedItems.filter((item) => item.section === 'my-zone');
  const managementItems = allowedItems.filter((item) => item.section === 'management');
  const systemItems = allowedItems.filter((item) => item.section === 'system');
  ...
  const sidebarContent = (
    <>
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-3">
        {/* MY ZONE Section */}
        {myZoneItems.length > 0 && (
          <div className="mb-6">
            {!isCollapsed && (
              <div className="px-4 mb-2 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-500 font-semibold">
                My Zone
              </div>
            )}
            <div className="space-y-1">
              {myZoneItems.map(renderNavItem)}
            </div>
          </div>
        )}
